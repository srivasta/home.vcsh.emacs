...
;; Last Modified On: Thu Oct 19 14:09:24 1989
;; Update Count    : 133
;; Status          : OK to use
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(setq file-header-update-alist
      (append '(("[ \t]Update Count[ \t]*: " . update-write-count)
		("[ \t]Last Machine Used: " . update-last-machine)
...
(defun make-header (&optional copyright)
  "Makes a standard file header at the top of the buffer. With prefix
 arg, also insert copyright and RCS keywords."
  (interactive "*P")
  (beginning-of-buffer)
	 ...
	 (comment-start-p (and comment-start
		...
		(cond
		 ...
		 (comment-start-p
		  ;; I will presume that spaces will fill the gaps
		  (concat comment-start
			  (make-string (- 79 (length comment-start))
					   ?\ )
			  ))
		 (t;; there is no known comment-start. Presume lisp
		  (concat ";;"
		...
		(concat "\n" (header-prefix-string)))
	       (year-string (substring (current-time-string) 20 24))
	       )
    (insert (concat
	     top-line
	     prefix-str
